...
export const Sidebar = ({ activeMenu = 'dashboard' }) => {
  ...
  // Auto-open recommendations dropdown if active menu is recommendations related
  useEffect(() => {
    if (activeMenu === 'recommendations' || activeMenu === 'recommendations-model' || activeMenu === 'recommendations-evaluation') {
  ...
  const menuItems = [
    ...
    { id: 'recommendations-group', icon: <Star size={18} />, label: 'Rekomendasi', path: '/admin/recommendations', hasDropdown: true},
      ...
      <div className="hidden md:flex md:w-60 bg-white h-screen flex-col shadow-lg">
        ...
        <nav className="flex-1 px-3 py-4">
          {menuItems.map(item => {
            ...
            // Menggunakan id 'recommendations-group' untuk dropdown
            if (item.hasDropdown && item.id === 'recommendations-group') {
              const isRecommendationsActive = activeMenu === 'recommendations' || activeMenu === 'recommendations-model' || activeMenu === 'recommendations-evaluation';
              return (
                <div key={item.id} className="mb-2">
                  {/* Parent Menu Item with Dropdown Indicator */}
                  <div
                    ...
                    onClick={() => setRecommendationsOpen(s => !s)}
                  >
                    <div className="flex items-center gap-3">
                      <div className={isRecommendationsActive ? 'text-white' : 'text-gray-600'}>
                        {item.icon}
                      </div>
                      <Text className={`font-medium ${isRecommendationsActive ? 'text-white' : 'text-gray-700'}`}>
                        {item.label}
                      </Text>
                    </div>
                    {/* Dropdown Indicator */}
                    <div className={isRecommendationsActive ? 'text-white' : 'text-gray-600'}>
                      {recommendationsOpen ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
                    </div>
                  </div>
                    ...
                    <div key={item.id}>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div className={`font-medium ${isRecommendationsActive ? 'text-white' : 'text-gray-700'}`}>
                            {item.label}
                          </div>
                        </div>
                        {/* Dropdown Indicator */}
                      ...
                      {recommendationsOpen && (
                        <div className="ml-4 pl-6 mt-2 border-l-2 border-gray-200 flex flex-col gap-1">
                          <button
                            onClick={() => { navigate('/admin/recommendations'); setMobileOpen(false); }}
                            className={`text-left px-3 py-2 rounded transition-all ${
                              activeMenu === 'recommendations'
                                ? 'bg-[#4782BE]/10 text-[#4782BE] font-medium border-l-2 border-[#4782BE] -ml-[2px]'
                                : 'text-gray-600 hover:bg-gray-100'
